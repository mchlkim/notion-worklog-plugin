...
- `태그`: `작업경로` 에서 파생한 슬러그. **같은 경로면 항상 같은 값**이어야 한다 —
  git 최상위(없으면 cwd)의 basename을 소문자·하이픈으로 정규화한다.
  `/Users/me/dev/api-server` → `api-server`. 규칙 전문은 [references/schema.md](references/schema.md) 의 "태그 규칙".
...
- `태그` 는 예외다. 경로에서 기계적으로 파생하는 값이므로 사용자에게 묻지 않는다.
